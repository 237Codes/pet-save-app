...
import { Users, Reports, Rewards, CollectedPets, Notifications, Transactions } from './schema';
...
export async function getCollectedPetsByCollector(collectorId: number) {
  try {
    return await db.select().from(CollectedPets).where(eq(CollectedPets.collectorId, collectorId)).execute();
  } catch (error) {
